...
function buildPaymentSchema(isCreditNote: boolean) {
  return z.object({
    amount: z
      ...
      .refine((val) => val !== 0, {
        message: isCreditNote ? 'El importe del abono debe ser distinto de 0' : 'El importe debe ser mayor que 0',
      })
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            Pendiente:{' '}
            <span className="font-semibold text-foreground">{formatCurrency(displayRemaining)}</span>
            {' de '}
            <span className="font-semibold text-foreground">{formatCurrency(Math.abs(invoiceTotal))}</span>
          </DialogDescription>
        </DialogHeader>
        ...
        {paymentType === PaymentType.FULL && (
          <div className="space-y-3 pt-1">
            ...
            >
              {createPayment.isPending ? 'Registrando...' : `${verb} ${formatCurrency(displayRemaining)}`}
            </Button>
          </div>
        )}
